...
//            'file' => $file,
    ...
    function attachments($id="") {
        ...
        $query="SELECT * FROM attachment WHERE usr_id=:email and status=0";
        if($id!=""){
         $query.=" and content_id='$id'" ;  
        }
        
        $sql = $this->db->select($query, $param);
        return $sql;
    }
    ...
    function user_id($data) {
        ...
        $sql = $this->db->select("SELECT id FROM registration WHERE (matricno=:email) limit 1", $param);
        
        return $sql[0]["id"];
    }

    function get_user() {
        $sql = $this->db->select("SELECT id,matricno,status FROM registration order by id desc ");
        return $sql;
    }
    ...
    function getUserDetails($id="",$status="") {
        $param = array(
            ':email' => $this->user_id(Session::get("email")),
        );
        $query="SELECT * FROM information WHERE usr_id=:email ";
        if($id !=""){
         $query.=" and id='$id'" ;  
        }
        if($status  !=""){
...
//$id = user_id($_SESSION["email"]);
        $sql = $this->db->select("SELECT save.id, matricno, title,cat, (
SELECT COUNT( attachment.id ) 
FROM attachment
WHERE save.id = attachment.content_id
) AS file_count, save_date, save_time, save.status,content
FROM information AS save, registration
WHERE  usr_id = registration.id and save.status=0
ORDER BY save.id DESC limit 20");
    ...
    function search($data) {
        $id = $data["param"];
        $sql = $this->db->select("SELECT a.id, title,cat, (
SELECT COUNT( attachment.id ) 
FROM attachment
...
WHERE (content like '%$id%'  or title like '%$id%' or cat like '%$id%' or b.description like '%$id%') 
    and b.name = a.cat and a.status=0
ORDER BY a.id DESC limit 20");
    ...
    function mysearch($data) {
        ...
        $id = $data["param"];
        $sql = $this->db->select("SELECT  distinct save.id, usr_id, title,cat, (
SELECT COUNT( attachment.id ) 
FROM attachment
WHERE save.id = attachment.content_id
) AS file_count, save_date, save_time, save.status,content,cat
FROM information AS save, registration, category as b
WHERE (content like '%$id%'  or title like '%$id%' or cat like '%$id%' or b.description like '%$id%') 
    and usr_id = registration.id
    ...
    function compileSearch($detail, $attached) {
        ...
        if (!empty($detail))
            foreach ($detail as $key => $value) {
                ...
                $acc.='<div class="feeds-title">' . $value["title"] ." ".$value["cat"] . '</div> ';
...
// $data2 = file_get_contents(URL . "public/.uploads/" . $data);
                        ...
                        $acc.='</object>';
                        $acc.='</div>';
                        $acc.='<div class="file-actions" style="display: block;  margin-top: 0; background-color: rgba(255,255,255,0.7);margin-left: 10px;">';
                        $acc.='<div class="file-footer-buttons">';
                        $acc.='<div class="kv-file-remove btn btn-xs btn-default hidden" title="Delete file"><a href="http://localhost/ElibraryGA/dashboard/delete/file"  file_id="' . $value1["id"] . '" class="delete"  ><i class="glyphicon glyphicon-trash text-danger"></a></i></div>';
                        $acc.='</div>';
                ...
                $acc.='   <div class="file-footer-buttons">';
                $acc.='<div class="kv-file-remove btn btn-xs btn-default hidden" title="Delete Document"><a  file_id="' . $value["id"] . '" class="delete" href="http://localhost/ElibraryGA/dashboard/delete/document"><i class="glyphicon glyphicon-trash text-danger"></i></a></div>';
                $acc.='</div>';
    ...
    function compileMySearch($detail, $attached) {
        ...
        if (!empty($detail))
            foreach ($detail as $key => $value) {
                ...
                $acc.='<div class = "class_category_header">'  . $value["title"] ." ".$value["cat"] . '</div>';
                ...
                foreach ($attached as $key1 => $value1) {
                    if ($value1["content_id"] == $value["id"]) {
                        ...
                        $acc.=' <div class = "file-footer-buttons">';
                        $acc.='<div class = "kv-file-remove btn btn-xs btn-default" title = "Delete file"><a href ="http://localhost/ElibraryGA/dashboard/delete/file"  file_id="' . $value1["id"] . '" class="delete"  ><i class="glyphicon glyphicon-trash text-danger"></a></i></div>';
                        $acc.='</div>';
                        $acc.='<div class="file-upload-indicator" title="Download File"><a href="' . URL . "public/.uploads/" . $data . ' target="__blank" class="download"><i class="glyphicon glyphicon-hand-down text-warning"></i></a></div>';
                        $acc.='<div class="clearfix"></div>';
                        $acc.='</div>';
                ...
                $acc.=' <div class="file-actions" style=" background-color: rgba(255,255,255,0.7);margin-left: 10px; padding: 20px;">';
                $acc.=' <div class="file-footer-buttons">';
                $acc.=' <div class="kv-file-remove btn btn-xs btn-default" title="Delete Document"><a  file_id="' . $value["id"] . '" class="delete" href="http://localhost/ElibraryGA/dashboard/delete/document"><i class="glyphicon glyphicon-trash text-danger"></i></a></div>';
                $acc.='</div>';
